(function(){
  ...
  // Android-only forced updater. Web/iOS have no AndroidUpdate bridge and are untouched.
  var CURRENT_TAG = 'v1.16.63';
  ...
  function saveRequired(info){
    try{ localStorage.setItem(CACHE_KEY, JSON.stringify(info)); }catch(_){ }
  }
  ...
  function setStatus(text){
    var e=document.getElementById('tf-force-update-status');
  ...
  async function check(force){
    ...
    if(cached && newer(cached.tag,CURRENT_TAG)){
      lock(cached);
    } else if(cached && !newer(cached.tag,CURRENT_TAG)){
      clearRequired();
    }
    var now=Date.now();
    try{
    ...
    try{
      ...
      // Fail-open only if this installation has never seen a mandatory version.
      // Once a newer required version has been cached, lock() above keeps the old APK unusable even offline.
