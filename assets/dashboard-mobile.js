...
// REV338: Candle Stick uses NET CHANGE from the selected Equity start balance.
// The first candle therefore opens at a true $0 baseline instead of floating
// around the account's starting capital (e.g. $5,000). Line Chart keeps using
// the original absolute account-balance values.
const tfCandleZeroUsdV338 = isCandleMode && equityMetric === 'usd';
const tfCandleStartBalanceV338 = (tfCandleZeroUsdV338 &&
  equityCurvePoints[0] && Number.isFinite(Number(equityCurvePoints[0].equity)))
  ? Number(equityCurvePoints[0].equity) : 0;
function tf_candleDisplayValueV338(v) {
  const n = Number(v);
  if (!Number.isFinite(n)) return 0;
  return tfCandleZeroUsdV338 ? (n - tfCandleStartBalanceV338) : n;
}
...
// REV338 Candle Stick: plot the account movement relative to the selected
// starting balance, with $0 as the visual origin. Use a dynamic "nice" scale
// instead of the old fixed $500 minimum step so small and large datasets both
// remain readable. If the curve crosses below the starting capital, keep $0
// centered in the visible range.
const tfZeroFloorUsdV326 = (equityMetric === 'usd' && Number(minYRaw) >= 0 &&
  (!compareSameMetric || !Number.isFinite(compareMinYRaw) || Number(compareMinYRaw) >= 0));
function tfNiceStepV338(rawStep) {
  const x = Math.max(1e-9, Math.abs(Number(rawStep) || 0));
...
if (tfCandleZeroUsdV338 && !useSharedActualLogScale) {
  // REV339: HARD ZERO FLOOR for USD Candle Stick.
  // The Y axis must always START at $0. Never create a symmetric negative
  // range (for example -$50K..+$50K) just because an early candle briefly
  // moves below the starting balance. Positive growth gets the full chart
  // height, while values below the starting balance are clipped at the $0
  // floor instead of forcing negative axis ticks.
  const hi = Math.max(0, Number(maxYRaw) || 0);
  const step = tfNiceStepV338(Math.max(hi, 1) / 4);
  tMin = 0;
  tMax = Math.max(step * 4, 1);
} else if (tfZeroFloorUsdV326 && !useSharedActualLogScale) {
  const rawTopV326 = Math.max(0, Number(maxYRaw) || 0) * 1.03;
  const niceStepV326 = Math.max(500, Math.ceil((rawTopV326 / 4) / 500) * 500);
  tMin = 0;
  tMax = niceStepV326 * 4;
} else if (tfZeroFloorUsdV326) {
  tMin = toT(0);
  const padTopV326 = Math.max(1e-9, (tMax - tMin) * 0.08);
  tMax += padTopV326;
} else if (tMin === tMax) {
...
// REV338: make the Candle Stick origin obvious. The first candle opens from
// this $0 line (net change relative to the selected starting balance).
...
const y = yForVal(Math.max(0, tf_candleDisplayValueV338(c.close)));
if (i === s)
ctx.moveTo(x, y);
else
ctx.lineTo(x, y);
