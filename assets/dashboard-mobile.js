...
// REV345: Percentage-based Equity Y-axis padding.
// Fixed dollar padding does not scale well across small and large accounts.
// The chart now adds 5% below the actual lower edge and 5% above the actual
// upper edge. Example: actual $5K..$10K => chart $4.75K..$10.5K.
...
if (equityMetric === 'usd' && !useSharedActualLogScale) {
  ...
  // $1K/$2K gap dominating a small visible range.
...
let ty;
if (isTouch) {
// REV347: Keep the detail popup outside the finger's occlusion zone.
// Prefer above the finger; if that cannot fit, use below, then the opposite side.
const clearance = 72;
...
if (pointX <= wrapRect.width / 2) {
tx = pointX + clearance;
}
else {
tx = pointX - tooltipRect.width - clearance;
}
}
}
}
else {
// Mouse/trackpad behavior remains unchanged for desktop/plugin parity.
tx = pointX + 8;
ty = pointY - 8;
}
...
tf_positionEquityTooltip(tooltip, wrapper, evt);
}
