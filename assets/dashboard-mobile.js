...
if (equityMetric === 'usd' && !useSharedActualLogScale) {
  ...
  // Do NOT inflate a small dataset into four forced $1K buckets. The visible
  // money range now hugs the actual data: one $1,000 gap below and one $1,000
  // gap above. Example: actual $5K..$10K => chart $4K..$11K.
  // If equity falls below the starting balance, extend the lower edge so data
  // is never clipped, while preserving the same $1,000 safety margin.
  const compareTopV344 = (compareSameMetric && Number.isFinite(compareMaxYRaw)) ? Number(compareMaxYRaw) : -Infinity;
  const actualLowV344 = Math.min(Number(minYRaw) || tfEqStartBalanceV340, tfEqStartBalanceV340);
  const actualTopV344 = Math.max(Number(maxYRaw) || tfEqStartBalanceV340, compareTopV344, tfEqStartBalanceV340);
  const globalMinV344 = actualLowV344 - 1000;
  const globalMaxV344 = actualTopV344 + 1000;

  // Keep REV343's readable Candle zoom behavior, but never create excessive
  // empty space. Full view uses the exact ±$1K outer padding above. During a
  // Candle zoom, the viewport may tighten further (up to a $1K local pad) so
  // daily OHLC bodies remain readable instead of flattening.
  if (isCandleMode && candleView && candleView.candles && candleView.candles.length) {
    const totalCandleV344 = Math.max(1, candleView.candles.length);
    const visibleCandleV344 = Math.max(1, candleView.end - candleView.start + 1);
    const visibleRatioV344 = Math.max(0, Math.min(1, visibleCandleV344 / totalCandleV344));
    const localLoV344 = Number(minYRaw);
    const localHiV344 = Number(maxYRaw);
    const localSpanV344 = Math.max(1, localHiV344 - localLoV344);
    const localPadV344 = Math.min(1000, Math.max(80, localSpanV344 * 0.12));
    let localMinV344 = localLoV344 - localPadV344;
    let localMaxV344 = localHiV344 + localPadV344;
    if (!(localMaxV344 > localMinV344)) {
      localMinV344 = localLoV344 - 100;
      localMaxV344 = localHiV344 + 100;
    }
    const zoomBlendV344 = Math.max(0, Math.min(1, (1 - visibleRatioV344) / 0.45));
    tMin = globalMinV344 * (1 - zoomBlendV344) + localMinV344 * zoomBlendV344;
    tMax = globalMaxV344 * (1 - zoomBlendV344) + localMaxV344 * zoomBlendV344;
    if (!(tMax > tMin)) {
      tMin = localMinV344;
      tMax = localMaxV344;
    }
...
// itself to four oversized buckets. Use clean $500/$1K/$2K/$5K/... tick steps,
// while the actual Y bounds remain exactly the requested outer padding.
if (equityMetric === 'usd' && !useSharedActualLogScale) {
  const rangeV344 = Math.max(1, tMax - tMin);
  const roughV344 = rangeV344 / 6;
  const powV344 = Math.pow(10, Math.floor(Math.log10(Math.max(1, roughV344))));
  const normV344 = roughV344 / powV344;
  let multV344 = 1;
  if (normV344 > 5) multV344 = 10;
  else if (normV344 > 2) multV344 = 5;
  else if (normV344 > 1) multV344 = 2;
  let tickStepV344 = multV344 * powV344;
  if (tickStepV344 < 500) tickStepV344 = 500;
  const epsV344 = tickStepV344 * 0.000001;
  const tickValsV344 = [];
  let firstTickV344 = Math.ceil((tMin - epsV344) / tickStepV344) * tickStepV344;
  for (let vV344 = firstTickV344; vV344 <= tMax + epsV344; vV344 += tickStepV344) {
    if (vV344 >= tMin - epsV344 && vV344 <= tMax + epsV344) tickValsV344.push(vV344);
    if (tickValsV344.length > 24) break;
  }
  if (tickValsV344.length < 3) {
    tickValsV344.unshift(tMin);
    tickValsV344.push(tMax);
  }
  const uniqueTicksV344 = [];
  tickValsV344.sort((a,b) => a-b).forEach((vV344) => {
    if (!uniqueTicksV344.length || Math.abs(vV344 - uniqueTicksV344[uniqueTicksV344.length - 1]) > 0.01) uniqueTicksV344.push(vV344);
  });
  uniqueTicksV344.forEach((value) => {
...
zoomSizingMode: 'visible-d1-count-plus-adaptive-height-v344',
