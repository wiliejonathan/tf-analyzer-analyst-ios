...
// REV341: lock the Candle Y scale to the COMPLETE selected dataset, not only
// the currently zoomed X viewport. Pinch/wheel zoom is therefore horizontal
// only: candle body/wick height does not change merely because the user zooms.
...
if (equityMetric === 'usd' && !useSharedActualLogScale) {
  ...
  tMin = tfEqAxisFloorV340;
  tMax = tfEqAxisFloorV340 + stepV340 * 4;
} else if (tMin === tMax) {
...
// Average candle step is retained for drag sensitivity only. Actual rendering
// uses the same trade-progression X domain as LINE Chart, so both modes show
// the same curve geometry instead of two different horizontal distributions.
const stepX = chartWidth / visCount;
function tf_candlePointStartV341(c) {
const v = Number(c && c.startPointIndex);
return Number.isFinite(v) ? Math.max(1, v) : 1;
}
function tf_candlePointEndV341(c) {
const v = Number(c && c.endPointIndex);
return Number.isFinite(v) ? Math.max(tf_candlePointStartV341(c), v) : tf_candlePointStartV341(c);
}
const firstVisibleCandleV341 = candles[s] || null;
const lastVisibleCandleV341 = candles[e] || firstVisibleCandleV341;
const viewPointStartV341 = Math.max(0, tf_candlePointStartV341(firstVisibleCandleV341) - 1);
const viewPointEndV341 = Math.max(viewPointStartV341 + 1, tf_candlePointEndV341(lastVisibleCandleV341));
const viewPointSpanV341 = Math.max(1, viewPointEndV341 - viewPointStartV341);
function tf_xForCandlePointV341(pointIndex) {
const p = Math.max(viewPointStartV341, Math.min(viewPointEndV341, Number(pointIndex) || viewPointStartV341));
return paddingLeft + ((p - viewPointStartV341) / viewPointSpanV341) * chartWidth;
}
function xForCandleAbsIndex(absIdx) {
const c = candles[absIdx] || null;
const a = tf_candlePointStartV341(c);
const b = tf_candlePointEndV341(c);
return tf_xForCandlePointV341((a + b) / 2);
}
function xForCandleCloseAbsIndex(absIdx) {
const c = candles[absIdx] || null;
return tf_xForCandlePointV341(tf_candlePointEndV341(c));
}
const candleCentersV341 = [];
for (let ci = s; ci <= e; ci++) candleCentersV341.push({ absIdx: ci, x: xForCandleAbsIndex(ci) });
function tf_candleWidthV341(absIdx) {
const x = xForCandleAbsIndex(absIdx);
let gap = Infinity;
if (absIdx > s) gap = Math.min(gap, Math.abs(x - xForCandleAbsIndex(absIdx - 1)));
if (absIdx < e) gap = Math.min(gap, Math.abs(xForCandleAbsIndex(absIdx + 1) - x));
if (!Number.isFinite(gap) || gap <= 0) gap = stepX;
// Keep candles readable on phones even with long histories. A small minimum
// width prevents them from collapsing into hairlines; zoom does not alter Y.
return Math.max(2.2, Math.min(12, gap * 0.70));
}
...
stepX: stepX,
visCount: visCount,
candleCenters: candleCentersV341,
viewPointStart: viewPointStartV341,
viewPointEnd: viewPointEndV341,
...
const candleW = tf_candleWidthV341(i);
const openY = yForVal(tf_candleDisplayValueV338(c.open));
const closeY = yForVal(tf_candleDisplayValueV338(c.close));
const highY = yForVal(tf_candleDisplayValueV338(c.high));
const lowY = yForVal(tf_candleDisplayValueV338(c.low));
...
// REV341: Mobile/Web Line/Candle X-domain parity + candle Y-scale lock during zoom.
