...
// REV326 mobile: when the USD Equity curve never goes below zero, anchor the
// visible Y-axis at a true $0 baseline. The previous symmetric padding could
// create an arbitrary bottom tick such as $180.27 even though the chart is
// conceptually measured from zero. Keep negative-equity datasets unchanged.
...
if (tfZeroFloorUsdV326 && !useSharedActualLogScale) {
  const rawTopV326 = Math.max(0, Number(maxYRaw) || 0) * 1.03;
  // Four grid intervals, rounded upward to the nearest $500. This yields clean
  // labels such as 0, 10K, 20K, 30K, 40K for a ~36K curve while still allowing
  // 1K / 1,5K / 2K / 2,5K style scales on smaller datasets.
