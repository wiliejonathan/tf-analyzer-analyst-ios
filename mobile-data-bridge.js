...
async function tfMobileRecoverRenderV31(reason){
  try{
    ...
    const historyRows=()=>document.querySelectorAll('#history-table tbody tr:not(.tf-start-balance-row)').length;
    const summaryRows=()=>document.querySelectorAll('#summary-table tbody tr').length;
    if(historyRows()>0||summaryRows()>0)return true;
    try{
    ...
    }catch(e){console.warn('TF Mobile V31 manual render recovery warning',reason,e);}
    return historyRows()>0||summaryRows()>0;
  }catch(e){console.warn('TF Mobile V31 render recovery failed',reason,e);return false;}
}
...
async function importFiles(files,opts={}){
  ...
  try{
    ...
    if(opts&&opts.reload===false){
      window.__tfMobileImportPendingReload=true;
      tfMobileHideImportLoading();
      return {ok:true,trades,analysts:expectedAnalysts,reload:false};
    }
