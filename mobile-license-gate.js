(() => {
  ...
  // REV332 — token gate before any Android / iOS PWA / browser feature is loaded.
  ...
  async function api(path, body, timeoutMs=25000) {
    ...
    try {
      const response=await fetch(API_BASE+path,{
        ...
        body:JSON.stringify({...body,deviceType:'MOBILE',clientType:'MOBILE',mobilePlatform:platformName(),mobileVersion:'1.0.101',remoteRevision:'REV332'})
      });
  ...
  async function startApp(){
    ...
    try{
      ...
      if(root)root.remove();
      setTimeout(()=>{try{if(typeof window.tfMobileRecoverRenderV31==='function')window.tfMobileRecoverRenderV31('token-gate-rev332');}catch(_){}},450);
  ...
  async function boot(){
    cleanupLegacyMobileApprovalArtifacts();
    createGate();bindUi();await restoreLoginBackup();
    const cred=getJson(CREDENTIALS_KEY);
    if(cred&&cred.email&&cred.token){showForm(cred);setStatus('Login tersimpan ditemukan. Menghubungkan server…');const ok=await login(cred.email,cred.token,{silent:true});if(ok)return;showForm(cred);return;}
