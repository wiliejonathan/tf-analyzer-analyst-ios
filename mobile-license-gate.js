(() => {
  ...
  // REV336 — persistent token gate: remembered login opens instantly; no activation screen on refresh.
  ...
  const APP_SCRIPTS = ['mobile-chrome-shim.js?rev=336','assets/dashboard-mobile.js?rev=336','mobile-data-bridge.js?rev=336','mobile-app-shell.js?rev=336','mobile-remote.js?rev=336'];
  ...
  async function api(path, body, timeoutMs=25000) {
    ...
    try {
      const response=await fetch(API_BASE+path,{
        ...
        body:JSON.stringify({...body,deviceType:'MOBILE',clientType:'MOBILE',mobilePlatform:platformName(),mobileVersion:'1.0.102',remoteRevision:'REV336'})
      });
  ...
  async function startApp(){
    ...
    try{
      ...
      if(root)root.remove();
      setTimeout(()=>{try{if(typeof window.tfMobileRecoverRenderV31==='function')window.tfMobileRecoverRenderV31('token-gate-rev336');}catch(_){}},450);
  ...
  async function boot(){
    ...
    // REV336: once Email + Token have been activated successfully, refresh/reopen
