(() => {
  ...
  const FAST_TICKET_CACHE_KEY='tfRemoteMobileFastTicketV279';
  ...
  async function api(path,body,timeoutOverrideMs){
    ...
    try{
      const r=await fetch(API+path,{method:'POST',cache:'no-store',signal:ctl?ctl.signal:undefined,headers:{'Content-Type':'application/json'},body:JSON.stringify({...a,...body,deviceType:'MOBILE',clientType:'MOBILE',mobileVersion:'1.0.98',remoteRevision:'REV298'})});
  ...
  async function getFastTicket(force=false){
    ...
    const x=await api('/remote/fast-ticket',{},6500);
  ...
  function scheduleFastReconnect(delay=900){
    if(document.hidden||fastReconnectTimer)return;
  ...
  async function connectFastLane(forceTicket=false){
    if(fastConnecting||fastSocketOpen()||document.hidden)return;
    fastConnecting=true;
  ...
  function disconnectRemotePresence(){
    ...
    try{
      fetch(API+'/remote/mobile-disconnect',{method:'POST',cache:'no-store',keepalive:true,headers:{'Content-Type':'application/json'},body:JSON.stringify({...a,deviceType:'MOBILE',clientType:'MOBILE',mobileVersion:'1.0.98',remoteRevision:'REV298'})}).catch(()=>{});
    }catch(_){}
  }
  ...
  function openRemote(resumeRestore=false){
    ensureUi();opened=true;failureCount=0;tfRemoteResumeRestoreV313=!!resumeRestore;bootHydrating=!resumeRestore;bootstrapOpenedAt=Date.now();
  ...
  function boot(){
    loadEvents();
    let restoreRemote=false;try{restoreRemote=localStorage.getItem(REMOTE_OPEN_KEY)==='1';}catch(_){}
    // Pre-warm WebSocket before the user taps Remote.
    setTimeout(()=>void connectFastLane(),30);
    const finishButton=()=>{const ok=ensureButton();if(ok&&restoreRemote)setTimeout(()=>{if(!opened)openRemote(true);},80);return ok;};
  ...
  window.addEventListener('pageshow',()=>{void connectFastLane();if(opened){const cached=loadBootstrapCache();if(cached){lastStatus=cached;try{render(lastStatus);}catch(_){}}setCommandOverlay(false);setSyncing(false);if(realtimeOpen()){sendMobileUiPresence(true);requestRealtimeSnapshot();}}},{passive:true});
