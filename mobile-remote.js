(() => {
  ...
  async function api(path,body,timeoutOverrideMs){
    ...
    try{
      const r=await fetch(API+path,{method:'POST',cache:'no-store',signal:ctl?ctl.signal:undefined,headers:{'Content-Type':'application/json'},body:JSON.stringify({...a,...body,deviceType:'MOBILE',clientType:'MOBILE',mobileVersion:'1.0.101',remoteRevision:'REV332'})});
  ...
  function disconnectRemotePresence(){
    ...
    try{
      fetch(API+'/remote/mobile-disconnect',{method:'POST',cache:'no-store',keepalive:true,headers:{'Content-Type':'application/json'},body:JSON.stringify({...a,deviceType:'MOBILE',clientType:'MOBILE',mobileVersion:'1.0.101',remoteRevision:'REV332'})}).catch(()=>{});
    }catch(_){}
  }
